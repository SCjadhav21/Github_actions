...
const Cssrepo = () => {
    ...
    let getData=()=>{
        ...
        axios.get(`https://api.github.com/search/repositories?q=stars:%3E1+language:css&per_page=10&page=${page}`)
 ...
 { data.map((res,index)=>{
    return <Box p="20px" color="#fff" bgColor="red" border="2px solid black" key={index}>
       ...
       <Box>Language : Css</Box>
        <Box>Forks count: {res.forks_count}</Box>
        <Box>Stars count: {res.stargazers_count}</Box>
        <Link to={`/card/${"css"}/${res.id}`}><Button color="red" bgColor="yellow" m="10px">View Details</Button></Link>
    </Box>
  })}
